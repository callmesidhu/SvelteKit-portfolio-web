<script lang="ts">
	...
	function cardTop(i: number) {
		return vh / 2 + cardY(i) - CARD_H / 2;
	}
...
<div bind:this={outer} class="exp-outer snap-start" style={isMobile ? '' : `height:${outerH}px`}>
		...
		<div class="track" style="transform:translateX(-{displayX}px); width:{trackW}px">
			...
			{#if loading}
				{#each Array(5) as _, i (i)}
					<div
						class="card skel"
						style="left:{PAD + i * STEP}px; top:{vh / 2 -
							CARD_H / 2}px; width:{CARD_W}px; height:{CARD_H}px"
					>
						<div class="sk l"></div>
						<div class="sk m mt2"></div>
						<div class="sk f mt4"></div>
						<div class="sk s"></div>
					</div>
				{/each}
			{:else}
				{#each experiences as exp, i (exp.id)}
					...
					>
						...
						{#if exp.skills?.length}
							<div class="skills">
								{#each exp.skills as sk (sk)}<span class="sk-tag">{sk}</span>{/each}
							</div>
						{/if}
					</div>
				{/each}
...
<style>
	...
	.ref-line {
		...
		box-shadow:
			0 0 20px 4px rgba(181, 138, 108, 0.18),
			0 0 60px 8px rgba(181, 138, 108, 0.08);
		pointer-events: none;
		z-index: 0;
	}
	...
	.card:hover {
		...
		box-shadow:
			0 0 40px rgba(181, 138, 108, 0.08),
	...
	.card-glow {
		...
		inset: 0;
		border-radius: 20px;
		background: radial-gradient(circle at 50% -20%, rgba(181, 138, 108, 0.12) 0%, transparent 70%);
		opacity: 0;
	...
	@media (max-width: 768px) {
		...
		.mob-dot {
			...
			background: #b58a6c;
			box-shadow:
				0 0 0 3px rgba(181, 138, 108, 0.2),
				0 0 10px rgba(181, 138, 108, 0.4);
		}
		.mob-num {
			font-size: 1.2rem;
			font-weight: 800;
