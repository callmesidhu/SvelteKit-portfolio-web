	...
	<div class="mt-8 grid grid-cols-1 gap-8 lg:grid-cols-2">
			...
			<div class="space-y-6">
				{#each Array(4) as _, i (i)}
					<div class="flex items-start gap-4">
						<div class="mt-2 h-2 w-2 rounded-full bg-admin-clay"></div>
						<div>
							<p class="text-sm text-admin-charcoal/90">
								Someone from New York viewed your <span class="text-admin-clay">Project AI</span>
							</p>
							<p class="mt-1 text-xs text-admin-charcoal/60">{i + 1} hour ago</p>
						</div>
					</div>
				{/each}
			</div>
		...
		>
			...
			<p class="mb-6 leading-relaxed text-admin-charcoal/80">
				Your portfolio is performing better than <span class="font-bold text-admin-charcoal">85%</span> of other
